...
function RdpClient(config) {
	...
	if (!config.proxy || config.proxy.disabled) {
	    this.bufferLayer = new layer.BufferLayer(new net.Socket());
	} else {
	    this.bufferLayer = new layer.BufferLayer(new socks.Socket(config.proxy));
	}
...
RdpClient.prototype.connect = function (host, port) {
	...
	this.bufferLayer.socket.connect(port, host);
	this.bufferLayer.socket.once('connect', function () {
